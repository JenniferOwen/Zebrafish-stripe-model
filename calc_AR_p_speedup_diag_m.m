function [Attract,Repulse]=calc_AR_p_speedup_diag_m(R,C,x_num,m_num,irr_num,max_distance,sizex,sizey, domain_matrix, domain_matrix_X, domain_matrix_irr)

%Function checks the surroundings of R,C (when R,C is on melanophore domain)
%and outputs matrices of the numbers of cells in each position - to be used 
%with the weighting vector (based on attraction/repulsion to cells to 
%determine probability of moving in each given direction

%Different to calc_AR_speedup_diag since requires search multiple sites on
%domains x and i.
...
%Counts the numbers of iridophores in each direction
Ni=0;
Si=0;
Ei=0;
Wi=0;
...
for i=-max_distance:1:max_distance
    for j=-max_distance:1:max_distance
        ...
        store_x= nnz(domain_matrix_X((2*R_check-1):2*R_check, (2*C_check-1):2*C_check)==x_num);
        store_i= nnz(domain_matrix_irr((2*R_check-1):2*R_check, (2*C_check-1):2*C_check)==irr_num);
        store_m= 4*domain_matrix(R_check, C_check)~=0;
        if i<=0
            if i<0
                Nm=Nm+store_m;
                Nx=Nx+store_x;
